...
export class SalaryService {
  ...
  async create(dto: CreateSalaryEntryDto, firmId: string, createdBy: string): Promise<SalaryEntry> {
    ...
    try {
      ...
      const salaryId = uuidv4();
      const groupId = uuidv4();
      const idempotencyBase = `freight-${firmId}-${dto.user_id}-${dto.salary_date}-${dto.amount}-${freightType}`;
      const description = dto.notes ?? `${freightLabel} paid to user ${dto.user_id}`;
      ...
      const firmCashDebit = qr.manager.create(LedgerEntry, {
        ...
        source_id: salaryId,
        entry_group_id: groupId,
        description: `${freightLabel} paid to user ${dto.user_id}`,
        idempotency_key: `${idempotencyBase}-firm-debit`,
        created_by: createdBy,
        user_id: dto.user_id,
      });
      ...
      await this.eventStore.publish({
        ...
        firm_id: firmId,
        aggregate_id: saved.id,
        payload: { user_id: dto.user_id, amount: dto.amount, salary_date: dto.salary_date, freight_type: freightType },
      });

      this.logger.log(`Freight [${freightType}] ${saved.id} paid to user ${dto.user_id}`);
      return saved;
    } catch (err) {
      await qr.rollbackTransaction();
      throw err;
  ...
  async findAll(
    firmId: string,
    filters: { user_id?: string; freight_type?: FreightType; from?: string; to?: string; page?: number; limit?: number },
  ): Promise<{ data: SalaryEntry[]; meta: object }> {
    const page = Math.max(1, Number(filters.page || 1));
    const limit = Math.min(100, Math.max(1, Number(filters.limit || 20)));
  ...
  async delete(id: string, firmId: string, deletedBy: string): Promise<void> {
    ...
    try {
      ...
      const firmCashCredit = qr.manager.create(LedgerEntry, {
        ...
        source_id: id,
        entry_group_id: groupId,
        description: `Reversal of ${freightLabel} entry ${id}`,
        idempotency_key: `${reversalBase}-firm-credit`,
        created_by: deletedBy,
        user_id: entry.user_id,
      });
